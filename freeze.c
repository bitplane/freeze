...
static char ident[] = "@(#) freeze.c 2.5.%d %s leo@ipmce.su\n";
...
long    refers_out = 0, symbols_out = 0;
#endif

/* Do not sleep when freeze works :-) */
long    indc_count, indc_threshold;
off_t   file_length = 0;	/* initial length of file */
...
/*****************************************************************
 *
 * Usage: freeze [-cdfivV] [-t] [-g...] [-x...] [type] [file ...]
 * Inputs:
 *
 *      -c:         Write output on stdout, don't remove original.
 *
 ...
 *                  form.  Useful with non-text files.  Default unless
 *                  TEXT_DEFAULT specified.
 *
 ...
 *                  if TEXT_DEFAULT specified.
 *
 *      -g:         Greedy parsing - increases speed, decreases
 *                  compression rate. Can be repeated.
 *
 *      -x:         "Anti- -g".
 *
 *      -v:         Write freezing statistics. -vv means "draw progress
...
/* use case-insensitive match: parent may not be command.com */
 ...
 /* 4.2BSD dependent - take it out if not */
  ...
  for (argc--, argv++; argc > 0; argc--, argv++) {
    if (**argv == '-' && (*argv)[1] != '-') {        /* A flag argument */
  ...
  if (do_melt) {
    ...
    setvbuf(stdout, (char*) hashtab, _IOFBF, (size_t) hash_size * sizeof(hash_t));
  } else {
	/* try to allocate all left memory for I/O buffers */
    size_t i = 64;      /* 512-byte blocks */
#ifdef SEGMENTED
    char * p;
    do {
      if ((p = malloc(i << 9)) != NULL)
	break;
      i--;
    } while (i >= 4);
    free(p);
    if (quiet < -1) fprintf(stderr, "%d blocks allocated\n", i);
#endif
    /* Don't check for failure - things cannot get worse than they were */
    setvbuf(stdin, NULL, _IOFBF, (i / 2) << 9);
    setvbuf(stdout, NULL, _IOFBF, (i - i / 2) << 9);
  }

  if (*filelist != NULL) {
    for (fileptr = filelist; *fileptr; fileptr++) {
      if ((**fileptr == '+' || **fileptr == '-') && do_melt == 0) {
	tune_table(*fileptr + 1 + (**fileptr == '-'));
	/* If a file type is given and no file names, a pipe
	 * operation is requested, but
	 * we will ignore an idiotic case of setting an resetting
	 * tables without any filename.
	 */
	if (filelist[1] == NULL)
	  goto Pipe;
	continue;
	...
	ofname[strlen(ofname) - 2] = '\0';    /* Strip off .F */
#ifdef DOS
...
#ifdef MSDOS
      /* freeze output always binary; melt output is binary if image ==
         O_BINARY */
...
Pipe:
    ...
    file_length = ((statbuf.st_mode & S_IFMT) == S_IFREG) ?
	statbuf.st_size : 0;
...
/* Deals with static Huffman table parameters.
	Input: command line option w/o leading `+' or `--'.
...
char   *type;
{
  ...
  /* both space-separated and comma-separated lists are allowed */
